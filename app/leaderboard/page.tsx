...
export default function LeaderboardPage() {
  ...
  useEffect(() => {
    ...
    // BroadcastChannel cross-tab listener for real-time live updates
    let bc: BroadcastChannel | null = null;
    if (typeof window !== "undefined" && "BroadcastChannel" in window) {
      try {
        bc = new BroadcastChannel("doodlebot_leaderboard_channel");
        bc.onmessage = () => {
          loadData();
        };
      } catch (err) {
    ...
    <ScreenShell showLogo={true}>
      <div className="flex flex-1 flex-col max-w-2xl mx-auto w-full p-4 space-y-6">
        ...
        <div className="flex-1 space-y-3">
          ...
          {isLoading ? (
            <div className="py-12 text-center text-sm font-semibold text-ink-muted animate-pulse">
